...
public class VehicleController {
    ...
    public List<VehicleResponseDto> list() { return service.list(); }

    @GetMapping("/{id}")
    @Operation(summary = "Detalha veículo")
    public VehicleResponseDto findById(@PathVariable Long id) { return service.findById(id); }

    @PostMapping
    @ResponseStatus(HttpStatus.CREATED)
    @Operation(summary = "Cria veículo")
    public VehicleResponseDto create(@RequestBody @Valid VehicleRequestDto request) { return service.create(request); }

    @PutMapping("/{id}")
    @Operation(summary = "Atualiza veículo")
    public VehicleResponseDto update(@PathVariable Long id, @RequestBody @Valid VehicleRequestDto request) { return service.update(id, request); }
